...
import { useSources, useMappingRules, useUnmappedLeads, useUnmappedSpend, useUnmappedSessions, useCreateMappingRule } from '@/hooks/useApi';
...
export default function SourcesAttribution() {
  ...
  const { data: unmappedLeads = [] } = useUnmappedLeads(f);
  const { data: unmappedSpend = [] } = useUnmappedSpend(f);
  const { data: unmappedSessions = [] } = useUnmappedSessions(f);
  ...
  return (
    <div className="space-y-4">
      ...
      {tab === 'sources' && (
        <div className="bg-card rounded-lg border border-border p-4">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-border text-muted-foreground uppercase tracking-wider text-[10px]"><th className="text-left py-2 px-2">Name</th><th className="text-left py-2 px-2">Type</th><th className="text-left py-2 px-2">Status</th></tr></thead>
            <tbody>
              {sources.map(s => (
                <tr key={s.id} className="border-b border-border/50 hover:bg-muted/30">
                  <td className="py-2 px-2 font-medium">{s.name}</td>
                  <td className="py-2 px-2 text-muted-foreground">{s.type}</td>
                  <td className="py-2 px-2"><span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${s.is_active ? 'bg-status-green/20 text-status-green' : 'bg-status-red/20 text-status-red'}`}>{s.is_active ? 'Active' : 'Disabled'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === 'rules' && (
        <div className="bg-card rounded-lg border border-border p-4">
          <table className="w-full text-xs">
            <thead><tr className="border-b border-border text-muted-foreground uppercase tracking-wider text-[10px]"><th className="text-left py-2 px-2">Input Type</th><th className="text-left py-2 px-2">Value</th><th className="text-left py-2 px-2">Maps To</th><th className="text-right py-2 px-2">Priority</th><th className="text-left py-2 px-2">Status</th></tr></thead>
            <tbody>
              {mappingRules.slice().sort((a, b) => b.priority - a.priority).map(r => (
                <tr key={r.id} className="border-b border-border/50 hover:bg-muted/30">
                  ...
                  <td className="py-2 px-2">{sources.find(s => s.id === r.source_id)?.name || r.source_id}</td>
                  <td className="text-right py-2 px-2">{r.priority}</td>
                  <td className="py-2 px-2"><span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${r.is_active ? 'bg-status-green/20 text-status-green' : 'bg-muted text-muted-foreground'}`}>{r.is_active ? 'On' : 'Off'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {tab === 'unmapped' && (
        <div className="space-y-4">
          <div className="bg-card rounded-lg border border-border p-4">
            ...
            {unmappedSessions.length === 0 ? <p className="text-xs text-muted-foreground">All sessions mapped! 🎉</p> : (
              <table className="w-full text-xs">
                <thead><tr className="border-b border-border text-muted-foreground uppercase tracking-wider text-[10px]"><th className="text-left py-2 px-2">Date</th><th className="text-left py-2 px-2">UTM Source</th><th className="text-left py-2 px-2">UTM Campaign</th><th className="text-left py-2 px-2">Landing Page</th><th className="text-left py-2 px-2">Action</th></tr></thead>
                <tbody>
                  {unmappedSessions.slice(0, 10).map(s => (
                    <tr key={s.id} className="border-b border-border/50">
                      ...
                      <td className="py-1.5 px-2"><button onClick={() => quickMap(s.utm_source, 'utm_source')} className="flex items-center gap-1 text-primary text-[10px] hover:underline"><Plus size={10} /> Map</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-card rounded-lg border border-border p-4">
            ...
            {unmappedLeads.length === 0 ? <p className="text-xs text-muted-foreground">All leads are mapped! 🎉</p> : (
              <table className="w-full text-xs">
                <thead><tr className="border-b border-border text-muted-foreground uppercase tracking-wider text-[10px]"><th className="text-left py-2 px-2">Date</th><th className="text-left py-2 px-2">UTM Source</th><th className="text-left py-2 px-2">UTM Campaign</th><th className="text-left py-2 px-2">Landing Page</th><th className="text-left py-2 px-2">Action</th></tr></thead>
                <tbody>
                  {unmappedLeads.slice(0, 20).map(l => (
                    <tr key={l.id} className="border-b border-border/50">
                      ...
                      <td className="py-1.5 px-2"><button onClick={() => quickMap(l.utm_source, 'utm_source')} className="flex items-center gap-1 text-primary text-[10px] hover:underline"><Plus size={10} /> Map</button></td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="bg-card rounded-lg border border-border p-4">
            ...
            {unmappedSpend.length === 0 ? <p className="text-xs text-muted-foreground">All spend mapped! 🎉</p> : (
              <table className="w-full text-xs">
                <thead><tr className="border-b border-border text-muted-foreground uppercase tracking-wider text-[10px]"><th className="text-left py-2 px-2">Date</th><th className="text-left py-2 px-2">Campaign</th><th className="text-left py-2 px-2">Platform</th><th className="text-right py-2 px-2">Amount</th></tr></thead>
                <tbody>
