...
import { fmt$, fmtP } from '@/services/metrics';
...
export default function ScorecardPage() {
  ...
  const selected = selectedId ? data.scorecards.find(s => s.id === selectedId) : null;
            ...
            {data.scorecards.map(sc => (
              <button key={sc.id} onClick={() => setSelectedId(sc.id)}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${gradeColor(sc.grade)}`}>
                    {sc.grade.toUpperCase()}
                  </span>
                </div>
          ...
          <div className="lg:col-span-2">
            {selected ? (
              <div className="bg-card rounded-lg border border-border p-5 space-y-4">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-sm font-bold">{selected.period === 'monthly' ? 'Monthly' : 'Weekly'} Scorecard</h3>
                    <p className="text-xs text-muted-foreground">{selected.start_date} → {selected.end_date} · {selected.owner}</p>
                  </div>
                  <span className={`px-2 py-1 rounded text-xs font-bold border ${gradeColor(selected.grade)}`}>
                    ...
                    {selected.action_items.map((ai, i) => (
                      <div key={i} className="flex items-start gap-2 text-xs bg-secondary/30 rounded p-2">
                        ...
                        <div className="flex-1">
                          <p className="font-medium">{ai.text}</p>
                          <p className="text-[10px] text-muted-foreground">{ai.owner} · Due {ai.due_date}</p>
                        </div>
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-medium ${ai.status === 'done' ? 'bg-status-green/20 text-status-green' : 'bg-secondary text-muted-foreground'}`}>
                          {ai.status}
                        </span>
                      </div>
                    ))}
        ...
        <div className="bg-card rounded-lg border border-border p-6 max-w-2xl mx-auto space-y-5">
          <div className="text-center border-b border-border pb-4">
            ...
            <p className="text-xs text-muted-foreground">
              {selected ? `${selected.start_date} → ${selected.end_date}` : 'Select a scorecard first'}
            </p>
          </div>
          ...
          {selected ? (
            <>
              <div>
                <h4 className="text-xs font-bold text-status-green flex items-center gap-1"><Trophy size={12} /> WINS</h4>
              ...
              <div>
                <h4 className="text-xs font-bold text-status-red flex items-center gap-1"><AlertTriangle size={12} /> LOSSES / CHALLENGES</h4>
                <ul className="mt-1 space-y-1 text-xs text-muted-foreground">
              ...
              <div>
                <h4 className="text-xs font-bold text-primary flex items-center gap-1"><MessageSquare size={12} /> DECISIONS NEEDED</h4>
                <ul className="mt-1 space-y-1 text-xs text-muted-foreground">
              ...
              <div>
                <h4 className="text-xs font-bold text-accent flex items-center gap-1"><CheckCircle2 size={12} /> ACTION ITEMS</h4>
